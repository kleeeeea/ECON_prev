...
#         new_d = []
#         for j, data_line in enumerate(data_list_line):
...
def filter_span_data_by_grammar(data, texts):
    new_data = []
    for line, spans in tqdm(zip(texts, data), total=len(texts)):
        if not spans:
        ...
        try:
            ...
            for index, token in enumerate(doc):
                ...
                tag_list[index] = token.tag_
            new_spans = []
            for span in spans:
                if 'IN' in tag_list[span['st']:span['ed']]:
                    continue
                elif tag_list[span['st']] == 'PRP$':
                    continue
                elif pos_list[span['ed']-1] not in ['NOUN', 'PROPN']:
                    continue
                else:
                    ind = span['st']
                    while ind < span['ed']:
                        if pos_list[ind] not in ['NOUN', 'ADV', 'ADJ', 'PROPN']:
                            ...
                            break
                    if ind < span['ed']:
                        new_span = {'st': ind, 'ed': span['ed'], 'text': ' '.join(
                            tokens[ind:span['ed']]), 'source': span['source']}
                        # new_spans.append(new_span)
                        add_span_to_spans(new_span, new_spans)
...
if __name__ == '__main__':
    ...
    outFilterFile = '{}/merge_data_filtered.json'.format(outDir)

    if not os.path.exists(outDir):
        os.makedirs(outDir)

    with open(outFilterFile, 'w') as fout:
        for new_d in new_data:
            fout.write(json.dumps(new_d))
            fout.write('\n')
